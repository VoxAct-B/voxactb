...
from rlbench import ObservationConfig2Robots
...
flags.DEFINE_string('mode',
                    'default',
                    'Task-specific mode. For example, "open_jar_close_to_jar" means we only record demonstrations starting with the left and right arms close to the jar.')
...
def save_demo(demo, example_path, variation):
    ...
    wrist2_rgb_path = os.path.join(example_path, WRIST2_RGB_FOLDER)
    wrist2_depth_path = os.path.join(example_path, WRIST2_DEPTH_FOLDER)
    wrist2_mask_path = os.path.join(example_path, WRIST2_MASK_FOLDER)
    ...
    check_and_make(wrist2_rgb_path)
    check_and_make(wrist2_depth_path)
    check_and_make(wrist2_mask_path)
    ...
    for i, obs in enumerate(demo):
        ...
        wrist2_rgb = Image.fromarray(obs.wrist2_rgb)
        wrist2_depth = utils.float_array_to_rgb_image(
            obs.wrist2_depth, scale_factor=DEPTH_SCALE)
        wrist2_mask = Image.fromarray((obs.wrist2_mask * 255).astype(np.uint8))
        ...
        wrist2_rgb.save(os.path.join(wrist2_rgb_path, IMAGE_FORMAT % i))
        wrist2_depth.save(os.path.join(wrist2_depth_path, IMAGE_FORMAT % i))
        wrist2_mask.save(os.path.join(wrist2_mask_path, IMAGE_FORMAT % i))
        ...
        obs.wrist_mask = None
        obs.wrist2_rgb = None
        obs.wrist2_depth = None
        obs.wrist2_point_cloud = None
        obs.wrist2_mask = None
        obs.front_rgb = None
...
def run(i, lock, task_index, variation_count, results, file_lock, tasks, task_name):
    ...
    obs_config.wrist2_camera.image_size = img_size
    obs_config.front_camera.image_size = img_size

    # Store depth as 0 - 1
    obs_config.wrist_camera.depth_in_meters = False
    obs_config.wrist2_camera.depth_in_meters = False
    obs_config.front_camera.depth_in_meters = False

    # We want to save the masks as rgb encodings.
    obs_config.wrist_camera.masks_as_one_channel = False
    obs_config.wrist2_camera.masks_as_one_channel = False
    obs_config.front_camera.masks_as_one_channel = False

    if FLAGS.renderer == 'opengl':
        obs_config.wrist_camera.render_mode = RenderMode.OPENGL
        obs_config.wrist2_camera.render_mode = RenderMode.OPENGL
        obs_config.front_camera.render_mode = RenderMode.OPENGL

    rlbench_env = Environment2Robots(
...
def run_all_variations(i, lock, task_index, variation_count, results, file_lock, tasks, task_name, seed):
    """Each thread will choose one task and variation, and then gather
    all the episodes_per_task for that variation."""
    if seed:
    ...
    while True:
        ...
        dominant = 'right'
        if FLAGS.mode in DOMINANT_ASSISTIVE_MODES:
            # start with left as dominant. After dominant_epsm switch to right as dominant
            dominant_eps = int(FLAGS.episodes_per_task / 2)
            dominant = 'left'

        for ex_idx in range(FLAGS.episodes_per_task):
            print(f'\n\n\n================ NEW EPISODE {ex_idx} ================')
            if FLAGS.mode in DOMINANT_ASSISTIVE_MODES and ex_idx >= dominant_eps:
                dominant = 'right'
            ...
            while attempts > 0:
                try:
                    ...
                    task_env = rlbench_env.get_task(t, dominant=dominant)
...
def main(argv):
    ...
    if FLAGS.all_variations:
        ...
        run_all_variations(0, lock, task_index, variation_count, result_dict, file_lock, tasks, task_name, seed)
    else:
        processes = [Process(
            target=run, args=(
