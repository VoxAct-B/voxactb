...
class Task2Robots(object):

    def __init__(self, pyrep: PyRep, robot_right_arm: Robot, robot_left_arm: Robot, name: str = None):
        """Constructor.
    ...
    def validate_dominant_assistive(self, dominant='right'):
        """If the task placement is valid. """
        self._waypoints = self._get_waypoints_dominant_assistive(False, dominant)
    ...
    def get_waypoints_dominant_assistive(self, dominant='right'):
        if self._waypoints is None:
            self._waypoints = self._get_waypoints_dominant_assistive(False, dominant)
        return self._waypoints
    ...
    def _feasible(self, waypoints: List[Point], waypoints_labels: List[str]) -> Tuple[bool, int]:
        has_left_arm_waypoint = False
        has_right_arm_waypoint = False
        for i, point in enumerate(waypoints):
            waypoint_label = waypoints_labels[i]

            if waypoint_label == 'left':
                left_arm = self.robot_left_arm.arm
                start_vals_left = left_arm.get_joint_positions()
                has_left_arm_waypoint = True
            elif waypoint_label == 'right':
                right_arm = self.robot_right_arm.arm
                start_vals_right = right_arm.get_joint_positions()
                has_right_arm_waypoint = True
            else:
                raise NotImplementedError
            ...
            if path is None:
                if waypoint_label == 'left':
                    left_arm.set_joint_positions(start_vals_left)
                elif waypoint_label == 'right':
                    right_arm.set_joint_positions(start_vals_right)
                return False, i
            path.set_to_end()
        # Needed twice otherwise can glitch out.
        if has_left_arm_waypoint:
            left_arm.set_joint_positions(start_vals_left)
        if has_right_arm_waypoint:
            right_arm.set_joint_positions(start_vals_right)
        return True, -1
    ...
    def _get_waypoints(self, validating=False) -> List[Waypoint]:
        ...
        # start with left arm first
        while True:
            name = waypoint_name_left % i
            if not Object.exists(name) or i == self._stop_at_waypoint_index:
                # NOTE a very weird bug in RLBench. Somehow waypoint#2, waypoint#3 cannot be found when we we use dataset_generator_two_robots.py
                if name == 'waypoint#3':
                    name = 'waypoint_hash3'
                elif name == 'waypoint#2':
                    name = 'waypoint_hash2'
            ...
            waypoints_labels.append('left')
            i += 1

        i = 0
        # collect waypoints for right arm
        while True:
            ...
            if ob_type == ObjectType.DUMMY:
                ...
                if i in self._waypoint_abilities_end:
                    end_func = self._waypoint_abilities_end[i]
                way = Point(waypoint, self.robot_right_arm,
                            start_of_path_func=start_func,
                            end_of_path_func=end_func)
            elif ob_type == ObjectType.PATH:
                cartestian_path = CartesianPath(name)
                way = PredefinedPath(cartestian_path, self.robot_right_arm)
            else:
                raise WaypointError(
                    '%s is an unsupported waypoint type %s' % (
                        name, ob_type), self)
            ...
            if name in self._waypoint_additional_inits and not validating:
                additional_waypoint_inits.append(
                    (self._waypoint_additional_inits[name], way))
            waypoints.append(way)
            waypoints_labels.append('right')
            i += 1

        # Check if all of the waypoints are feasible
        feasible, way_i = self._feasible(waypoints, waypoints_labels)
        if not feasible:
            raise WaypointError(
        ...
        return waypoints, waypoints_labels

    def _get_waypoints_dominant_assistive(self, validating=False, dominant='right') -> List[Waypoint]:
        waypoint_name_left = 'waypoint#%d'
        waypoint_name = 'waypoint%d'
        waypoints = []
        waypoints_labels = []
        ...
        print('What is the dominant arm in _get_waypoints_dominant_assistive: ', dominant)
        if dominant == 'right':
            assistive_robot_arm = self.robot_left_arm
            dominant_robot_arm = self.robot_right_arm
            assistive_label = 'left'
            dominant_label = 'right'
        else:
            # dominant is left
            assistive_robot_arm = self.robot_right_arm
            dominant_robot_arm = self.robot_left_arm
            assistive_label = 'right'
            dominant_label = 'left'

        # start with assistive arm first
        while True:
            name = waypoint_name_left % i
            if not Object.exists(name) or i == self._stop_at_waypoint_index:
                # NOTE a very weird bug in RLBench. Somehow waypoint#2, waypoint#3 cannot be found when we we use dataset_generator_two_robots.py
            ...
            if ob_type == ObjectType.DUMMY:
                ...
                if i in self._waypoint_abilities_end:
                    end_func = self._waypoint_abilities_end[i]
                way = Point(waypoint, assistive_robot_arm,
                            start_of_path_func=start_func,
                            end_of_path_func=end_func)
            elif ob_type == ObjectType.PATH:
                cartestian_path = CartesianPath(name)
                way = PredefinedPath(cartestian_path, assistive_robot_arm)
            else:
                raise WaypointError(
                    '%s is an unsupported waypoint type %s' % (
                        name, ob_type), self)
            ...
            if name in self._waypoint_additional_inits and not validating:
                additional_waypoint_inits.append(
                    (self._waypoint_additional_inits[name], way))
            waypoints.append(way)
            waypoints_labels.append(assistive_label)
            i += 1

        i = 0
        # collect waypoints for dominant arm
        while True:
            name = waypoint_name % i
            if not Object.exists(name) or i == self._stop_at_waypoint_index:
                # There are no more waypoints...
            ...
            if ob_type == ObjectType.DUMMY:
                ...
                if i in self._waypoint_abilities_end:
                    end_func = self._waypoint_abilities_end[i]
                way = Point(waypoint, dominant_robot_arm,
                            start_of_path_func=start_func,
                            end_of_path_func=end_func)
            elif ob_type == ObjectType.PATH:
                cartestian_path = CartesianPath(name)
                way = PredefinedPath(cartestian_path, dominant_robot_arm)
            else:
                raise WaypointError(
                    '%s is an unsupported waypoint type %s' % (
                        name, ob_type), self)
            ...
            if name in self._waypoint_additional_inits and not validating:
                additional_waypoint_inits.append(
                    (self._waypoint_additional_inits[name], way))
            waypoints.append(way)
            waypoints_labels.append(dominant_label)
            i += 1
