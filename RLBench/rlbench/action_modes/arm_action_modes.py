...
class EndEffectorPoseViaPlanning(ArmActionMode):
    ...
    def _quick_boundary_check(self, scene: Scene, action: np.ndarray):
        ...
        if not scene.check_target_in_workspace(pos_to_check):
            print('A path could not be found because the target is outside of workspace.')
        ...
        try:
            ...
            try:
                ...
                if ignore_collisions:
                    print('A path could not be found. Most likely due to the target '
                        'being inaccessible or a collison was detected.')
                    raise InvalidActionError(
...
class EndEffectorPoseViaPlanning2Robots(ArmActionMode):
    """High-level action where target pose is given and reached via planning.
    ...
    def _quick_boundary_check(self, scene: Scene, action: np.ndarray, robot: Robot):
        pos_to_check = action[:3]
    ...
    def action(self, scene: Scene, action: np.ndarray, ignore_collisions: bool = True, which_arm: str = None):
        assert_action_shape(action, (7,))
        assert_unit_quaternion(action[3:])
        ...
        if which_arm == 'right':
            ...
            print(f'which_arm: {which_arm}') # for debugging
            raise NotImplementedError

        if not self._absolute_mode and self._frame != 'end effector':
            action = calculate_delta_pose(curr_arm, action)
        relative_to = None if self._frame == 'world' else curr_arm.arm.get_tip()
        self._quick_boundary_check(scene, action, curr_arm)

        colliding_shapes = []
        if not ignore_collisions:
            if self._robot_shapes is None:
                self._robot_shapes = curr_arm.arm.get_objects_in_tree(
                    object_type=ObjectType.SHAPE)
            # First check if we are colliding with anything
            colliding = curr_arm.arm.check_arm_collision()
            if colliding:
                # Disable collisions with the objects that we are colliding with
                grasped_objects = curr_arm.gripper.get_grasped_objects()
                colliding_shapes = [
                    s for s in scene.pyrep.get_objects_in_tree(
                        object_type = ObjectType.SHAPE) if (
                            s.is_collidable() and
                            s not in self._robot_shapes and
                            s not in grasped_objects and
                            curr_arm.arm.check_arm_collision(
                                s))]
                [s.set_collidable(False) for s in colliding_shapes]
...
class EndEffectorPoseViaIK(ArmActionMode):
    ...
    def action(self, scene: Scene, action: np.ndarray):
        ...
        try:
            ...
            print('Could not perform IK via Jacobian; most likely due to current '
