...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        self.arm_pred_loss = arm_pred_loss
        ...
        if self.num_rotation_classes > 0:
            ...
            # arm MLP layer
            if self.arm_pred_loss:
                self.dense2 =  DenseBlock(flat_size, self.final_dim, None, activation)
                # left arm or right arm
                self.arm_ff = DenseBlock(self.final_dim, 2, None, None)
    ...
    def forward(
        ...
        if self.num_rotation_classes > 0:
            ...
            if self.arm_pred_loss:
                # left arm or right arm prediction
                dense2 = self.dense2(torch.cat(feats, dim=1))
                arm_out = self.arm_ff(dense2)
                return trans, rot_and_grip_out, collision_out, arm_out
...
class PerceiverVoxelLang2RobotsEncoder(nn.Module):
    def __init__(
        ...
        if self.num_rotation_classes > 0:
            ...
            self.rot_grip_collision_ff = DenseBlock(self.final_dim,
                                                    ...
                                                    self.num_collision_classes,
                                                    None, None)
        self.trans_decoder_left_arm = Conv3DBlock(
            self.final_dim, 1, kernel_sizes=3, strides=1,
            norm=None, activation=None,
        )
        self.ss_final_left_arm = SpatialSoftmax3D(
            self.voxel_size, self.voxel_size, self.voxel_size,
            self.im_channels)
        self.dense0_left_arm =  DenseBlock(flat_size, 256, None, activation)
        self.dense1_left_arm = DenseBlock(256, self.final_dim, None, activation)
        self.rot_grip_collision_ff_left_arm = DenseBlock(self.final_dim,
                                                self.num_rotation_classes * 3 + \
                                                self.num_grip_classes + \
                                                self.num_collision_classes,
                                                None, None)
    ...
    def forward(
        ...
        # translation decoder
        trans_right = self.trans_decoder(u)

        # rotation, gripper, and collision MLPs
        rot_and_grip_out_right = None
        if self.num_rotation_classes > 0:
            feats.extend([self.ss_final(u.contiguous()), self.global_maxp(u).view(b, -1)])
            ...
            rot_and_grip_collision_out = self.rot_grip_collision_ff(dense1)
            rot_and_grip_out_right = rot_and_grip_collision_out[:, :-self.num_collision_classes]
            collision_out_right = rot_and_grip_collision_out[:, -self.num_collision_classes:]

        ############# heads for left arm #############
        # translation decoder
        trans_left = self.trans_decoder_left_arm(u)

        # rotation, gripper, and collision MLPs
        rot_and_grip_out_left = None
        if self.num_rotation_classes > 0:
            # remove feats from right arm's ss_final and global_maxp
            feats = feats[:-2]
            feats.extend([self.ss_final_left_arm(u.contiguous()), self.global_maxp(u).view(b, -1)])

            dense0 = self.dense0_left_arm(torch.cat(feats, dim=1))
            dense1 = self.dense1_left_arm(dense0)                     # [B,72*3+2+2]

            rot_and_grip_collision_out = self.rot_grip_collision_ff_left_arm(dense1)
            rot_and_grip_out_left = rot_and_grip_collision_out[:, :-self.num_collision_classes]
            collision_out_left = rot_and_grip_collision_out[:, -self.num_collision_classes:]

        return trans_right, rot_and_grip_out_right, collision_out_right, trans_left, rot_and_grip_out_left, collision_out_left
