...
from rlbench import CameraConfig, ObservationConfig2Robots
from pyrep.const import RenderMode
from typing import List

REMOVE_KEYS = ['joint_velocities_right', 'joint_positions_right', 'joint_forces_right',
               ...
               'task_low_dim_state', 'misc', 'target_object_pos']

SCALE_FACTOR = DEPTH_SCALE
DEFAULT_SCENE_SCALE = 2.0

def extract_left_and_right_arm_instruction(description):
    descs = description.split(' and ')
    left_arm_description = descs[0]
    right_arm_description = descs[-1]
    assert 'left hand' in left_arm_description and 'right hand' not in left_arm_description
    assert 'right hand' in right_arm_description and 'left hand' not in right_arm_description
    return left_arm_description, right_arm_description
...
def extract_obs(obs: Observation2Robots,
                cameras,
                t: int = 0,
                prev_action=None,
    ...
    if which_arm == 'right':
        robot_state = np.array([
            obs.gripper_right_open,
            *obs.gripper_right_joint_positions])
    elif which_arm == 'left':
        robot_state = np.array([
            obs.gripper_left_open,
            *obs.gripper_left_joint_positions])
    ...
    if keypoint_label is None:
        ...
        # keypoint_label is not None, which means arm_id_to_proprio is True
        time = (1. - (t / float(episode_length - 1))) * 2. - 1.
        obs_dict['low_dim_state'] = np.concatenate(
                [obs_dict['low_dim_state'], [time], [keypoint_label]]).astype(np.float32)
        # print('Arm id: ', keypoint_label) # for debugging
    else:
        # arm_pred_input is true, which replaces timestep with current arm ID for the multi-arm policy
        obs_dict['low_dim_state'] = np.concatenate(
