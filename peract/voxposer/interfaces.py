...
EE_ALIAS = ['ee', 'endeffector', 'end_effector', 'end effector', 'gripper', 'hand', 'left hand', 'right hand']
TABLE_ALIAS = ['table', 'desk', 'workstation', 'work_station', 'work station', 'workspace', 'work_space', 'work space']
DIRTS = ['dirt0', 'dirt1', 'dirt2', 'dirt3', 'dirt4']
...
class LMP_interface():
  ...
  def __init__(self, env, lmp_config, controller_config, planner_config, env_name='rlbench'):
    ...
    self._target_object_world_coords = None
    self.voxposer_only_eval = False
  ...
  # ======================================================
  def get_ee_pos(self, obj_name):
    return self._world_to_voxel(self._env.get_ee_pos(obj_name))
  ...
  def detect(self, obj_name):
    ...
    if obj_name.lower() in EE_ALIAS:
      ...
      obs_dict['position'] = self.get_ee_pos(obj_name)
      obs_dict['aabb'] = np.array([self.get_ee_pos(obj_name), self.get_ee_pos(obj_name)])
      obs_dict['_position_world'] = self._env.get_ee_pos(obj_name)
      self._current_arm = obj_name
  ...
  def execute(self, movable_obs_func, affordance_map=None, avoidance_map=None, rotation_map=None,
    ...
    # NOTE: modify this when new task is added (optinoal, depending on task)
    if self._env.task._task.__class__.__name__ == 'SweepToDustpan' and isinstance(movable_obs_func(), list):
      dirts = movable_obs_func()
      dirts_pc = []
      for dirt in dirts:
         dirts_pc.append(dirt['_point_cloud_world'])
      dirts_pc = np.concatenate(dirts_pc, axis=0)
      self._target_object_world_coords = np.mean(dirts_pc, axis=0)
      # return nothing since we're only updating self._target_object_world_coords
      return []
    ...
    if affordance_map is not None:
      ...
      for plan_iter in range(self._cfg['max_plan_iter']):
        ...
        if self.voxposer_only_eval:
          if self._env.task._task.__class__.__name__ == 'OpenJar':
            traj_world = traj_world[-10:]
            print('Use fewer waypoints (10)!!')
        else:
          if self._env.task._task.__class__.__name__ == 'OpenDrawer' or self._env.task._task.__class__.__name__ == 'PutItemInDrawer':
            # only use the last few waypoints because doing intermediate waypoints for OpenDrawer does not work
            # as well due to collisions and bad positions for end pose, which could cause it to not reach the end pose
            traj_world = traj_world[-5:]
            print('Use fewer waypoints!!')
          elif self._env.task._task.__class__.__name__ == 'OpenJar':
            traj_world = traj_world[-10:]
            print('Use fewer waypoints!!')
    ...
    if not object_centric:
      try:
        ...
        _gripper_map = gripper_map()
        # get last ee pose
        ee_pos_world = self._env.get_ee_pos(self._current_arm)
        ee_pos_voxel = self.get_ee_pos(self._current_arm)
      ...
      self._env.apply_action(np.concatenate([ee_pose_world, [gripper_state]]), self._current_arm)
  ...
  def set_voxposer_only_eval(self, flag):
    self.voxposer_only_eval = flag
  ...
  def _get_default_voxel_map(self, type='target'):
    ...
    def fn_wrapper():
      if type == 'target':
        ...
        voxel_map = np.ones((self._map_size, self._map_size, self._map_size)) * self._env.get_last_gripper_action(self._current_arm)
      elif type == 'rotation':
        voxel_map = np.zeros((self._map_size, self._map_size, self._map_size, 4))
        voxel_map[:, :, :] = self._env.get_ee_quat(self._current_arm)
      else:
...
def setup_LMP(env, general_config, debug=False, cache_dir='cache', voxposer_only_eval=False):
  ...
  composer = LMP(
      'composer', lmps_config['composer'], fixed_vars, variable_vars, debug, env_name, cache_dir=cache_dir, voxposer_only_eval=voxposer_only_eval
  )
  variable_vars['composer'] = composer

  # creating the LMP that deals w/ high-level language commands
  task_planner = LMP(
      'planner', lmps_config['planner'], fixed_vars, variable_vars, debug, env_name, cache_dir=cache_dir, voxposer_only_eval=voxposer_only_eval
  )
  ...
  lmp_env.set_voxposer_only_eval(voxposer_only_eval)
