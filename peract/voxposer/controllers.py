...
EE_ALIAS = ['ee', 'endeffector', 'end_effector', 'end effector', 'gripper', 'hand', 'right hand', 'left hand']
...
class Controller:
    ...
    def _apply_mpc_control(self, control, target_velocity=1, movable_name=None):
        """
        ...
        # apply control
        self.env.close_gripper(movable_name)
        # move to start pose
        self.env.move_to_pose(np.concatenate([t_start, ee_quat]), movable_name, speed=target_velocity)
        print('[controllers.py] moved to start pose', end='; ')
        # move to interact pose
        self.env.move_to_pose(np.concatenate([t_interact, ee_quat]), movable_name, speed=target_velocity * 0.2)
        print('[controllers.py] moved to final pose', end='; ')
        # back to rest pose
        self.env.move_to_pose(np.concatenate([t_rest, ee_quat]), movable_name, speed=target_velocity * 0.33)
    ...
    def execute(self, movable_obs, waypoint):
        ...
        :return: None
        """
        movable_name = movable_obs['name'].lower()
        info = dict()
        ...
        if not object_centric:
            ...
            result = self.env.apply_action(np.concatenate([target_pose, [target_gripper]]), movable_name)
            info['mp_info'] = result
        # optimize through dynamics model to obtain robot actions
            ...
            self._apply_mpc_control(best_control[0], movable_name=movable_name)
