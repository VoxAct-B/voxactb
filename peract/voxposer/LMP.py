...
class LMP:
    ...
    def __init__(self, name, cfg, fixed_vars, variable_vars, debug=False, env='rlbench', cache_dir='../../../../voxposer/cache', voxposer_only_eval=False):
        self._name = name
        self._cfg = cfg
        self._debug = debug
        ...
        self.exec_hist = ''
        self._context = None
        self._cache = DiskCache(cache_dir=cache_dir, load_cache=self._cfg['load_cache'])
    ...
    def build_prompt(self, query):
        # TODO: need a better way to adjust the query for VoxPoser
        # NOTE: modify this when new task is added
        if self._voxposer_only_eval is False and query == 'grasp the jar with left hand and grasp the lid of the jar with right hand to unscrew it in an anti_clockwise direction until it is removed from the jar':
            # extract left hand and right hand actions from query and discard everything else
            query = query.split('right hand')[0] + 'right hand'
        if self._voxposer_only_eval is False and query == 'grasp the jar with right hand and grasp the lid of the jar with left hand to unscrew it in an anti_clockwise direction until it is removed from the jar':
            # extract left hand and right hand actions from query and discard everything else
            query = query.split('left hand')[0] + 'left hand'
        if self._voxposer_only_eval is False and query == 'hold the drawer with left hand and open the bottom drawer with right hand':
            query = 'get close to the top drawer with left hand and get close to the bottom drawer with right hand'
        if self._voxposer_only_eval is False and query == 'hold the drawer with right hand and open the bottom drawer with left hand':
            query = 'get close to the top drawer with right hand and get close to the bottom drawer with left hand'
        if self._voxposer_only_eval is False and query == 'hold the dustpan with left hand and grasp the broom with right hand to brush the dirt into the dustpan':
            query = 'find all dirts and get close to the dustpan with left hand and get close to the broom with right hand'
        if self._voxposer_only_eval is False and query == 'open the top drawer with right hand and put the item in the top drawer with left hand':
            query = 'get close to the top drawer handle with right hand and get close to the item with left hand'
        if self._voxposer_only_eval is False and query == 'open the top drawer with left hand and put the item in the top drawer with right hand':
            query = 'get close to the top drawer handle with left hand and get close to the item with right hand'
        ...
        if kwargs['model'] != 'gpt-3.5-turbo-instruct' and \
            ...
            if kwargs in self._cache:
                ...
                try:
                    ...
                    ret = openai.ChatCompletion.create(**kwargs)['choices'][0]['message']['content'] # do the same call again
                # post processing
