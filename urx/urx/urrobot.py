...
class URRobot(object):
    ...
    def activate_gripper(self):
        print('Sending gripper activation sequence...')
        self.set_tool_voltage(24)
        self.set_digital_out(8, False)
        self.set_digital_out(9, False)
        time.sleep(0.05)

        self.set_digital_out(8, True)
        time.sleep(0.05)

        self.set_digital_out(8, False)
        time.sleep(0.05)

        self.set_digital_out(9, True)
        time.sleep(0.05)

        self.set_digital_out(9, False)
        time.sleep(0.05)
        print("Finish gripper's activation sequence.")

    def get_gripper_position(self):
        dout0 = int(self.get_digital_out(8))
        dout1 = int(self.get_digital_out(9))
        if dout0 == 0 and dout1 == 0:
            # open
            val = 0
        elif dout0 == 1 and dout1 == 0:
            # close
            val = 255
        return val
